        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <a href="{{ route('owner.consultations.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md">
                ...
                @if($consultations->isEmpty())
                    <div class="text-center py-8">
                        ...
                        <a href="{{ route('owner.consultations.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-6 py-2 rounded-md inline-block">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($consultations as $consultation)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">Dr. {{ $consultation->doctor->name }}</div>
                                            <div class="text-sm text-gray-500">{{ $consultation->doctor->specialization ?? 'General Practice' }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('owner.consultations.show', $consultation) }}" class="text-blue-600 hover:text-blue-900">View Details</a>
                                            @if($consultation->status === 'pending')
                                                <form action="{{ route('owner.consultations.destroy', $consultation) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="ml-2 text-red-600 hover:text-red-900" onclick="return confirm('Are you sure you want to cancel this consultation?')">
                                                        Cancel
                                                    </button>
                                                </form>
                                            @endif
